...
import { ArrowLeft, Bell, Sparkles, Check, Play, Gamepad2, HelpCircle, Puzzle, Lock, ArrowRight, BookOpen, Trophy, X, FileText, Star, Settings } from 'lucide-react';
...
export const LearningPathScreen: React.FC<Props> = ({ onStartQuiz, onUpgradeUnit, onStartComprehensiveTest, onBack, user, onSettings }) => {
      ...
      <div className="sticky top-0 z-50 bg-white/70 dark:bg-dark-bg/60 backdrop-blur-md border-b border-teal-100 dark:border-teal-800 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-3">
          {onSettings && (
            <button
              onClick={onSettings}
              className="flex items-center gap-2 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:hover:bg-red-900/30 px-3 py-1.5 rounded-full border border-red-200 dark:border-red-800 transition-all animate-pulse"
            >
              <div className="p-1 bg-red-500 rounded-full text-white">
                <Settings size={12} />
              </div>
              <span className="text-[10px] font-bold text-red-600 dark:text-red-400 hidden sm:inline-block">Lấy API key để sử dụng app</span>
            </button>
          )}
