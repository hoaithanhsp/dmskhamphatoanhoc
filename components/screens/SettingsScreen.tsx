...
import { ArrowLeft, Edit, Brain, UserCog, ChevronRight, Bell, Globe, Shield, Info, FileText, LogOut, X, School, CheckCircle, Trophy, Key, Save } from 'lucide-react';
...
export const SettingsScreen: React.FC<Props> = ({ user, onLogout, onBack }) => {
    ...
    const [activeModel, setActiveModel] = useState(() => localStorage.getItem('PREFERRED_MODEL') || 'gemini-3-flash-preview');
    ...
    return (
        <div className="bg-primary-surface dark:bg-dark-bg font-display text-gray-900 dark:text-white antialiased min-h-screen flex flex-col relative">
            ...
            <main className="flex-1 flex flex-col gap-6 p-4 w-full">
                ...
                {/* AI Configuration */}
                <section>
                    <h3 className="px-2 pb-2 text-xs font-bold text-gray-500 uppercase tracking-wider">Cấu hình AI</h3>
                    <div className="bg-white dark:bg-surface-dark rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col gap-4">
                        {/* API Key Input */}
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                ...
                                <a href="https://tinyurl.com/hdsdpmTHT" target="_blank" rel="noopener noreferrer" className="text-[10px] text-primary hover:underline flex items-center gap-1">
                                    <Info size={10} /> Hướng dẫn
                                </a>
                            </div>
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-dark-bg px-3 py-2 text-sm focus:ring-2 focus:ring-primary outline-none transition-all dark:text-white"
                                />
                                <button
                                    onClick={handleSaveApiKey}
                                    className="bg-primary text-white p-2 rounded-xl hover:bg-primary-dark transition-colors flex items-center justify-center min-w-[40px]"
                                >
                                    <Save size={18} />
                                </button>
                            </div>
                            <div className="mt-2 text-xs text-gray-500">
                                Chưa có key? <a href="https://aistudio.google.com/api-keys" target="_blank" rel="noopener noreferrer" className="text-primary font-bold hover:underline">Lấy tại đây</a>
                            </div>
                        </div>
                        ...
                        {/* Model Selection */}
                        <div>
                            <label className="block text-sm font-medium text-gray-900 dark:text-white mb-2">Chọn Model Ưu Tiên</label>
                            <div className="grid grid-cols-1 gap-2">
                                {['gemini-3-flash-preview', 'gemini-3-pro-preview', 'gemini-2.5-flash'].map(model => (
                                    <button
                                        key={model}
                                        onClick={() => handleSaveModel(model)}
                                        className={`relative flex items-center p-3 rounded-xl border text-left transition-all ${activeModel === model
                                                ? 'border-primary bg-primary/5 ring-1 ring-primary'
                                                : 'border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-white/5'
                                            }`}
                                    >
                                        <div className={`p-2 rounded-full mr-3 ${activeModel === model ? 'bg-primary text-white' : 'bg-gray-100 text-gray-500 bg-white dark:bg-gray-800'}`}>
                                            <Brain size={16} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <span className={`block text-sm font-bold truncate ${activeModel === model ? 'text-primary' : 'text-gray-700 dark:text-gray-300'}`}>
                                                {model}
                                            </span>
                                            {model === 'gemini-3-flash-preview' && <span className="text-[10px] text-gray-500 block">Khuyên dùng (Tốc độ cao)</span>}
                                            {model === 'gemini-3-pro-preview' && <span className="text-[10px] text-gray-500 block">Thông minh hơn</span>}
                                        </div>
                                        {activeModel === model && <div className="absolute right-3 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-primary shadow-glow"></div>}
                                    </button>
                                ))}
                            </div>
                        </div>
